...
async def cleanup_old_alerts(days_closed: int = 30, days_open: int = 90, days_low: int = 7, days_benign: int = 3):
    """
    Limpiar alertas antiguas de la base de datos.
    ...
    try:
        ...
        now = datetime.now()
        date_closed = now - timedelta(days=days_closed)
        date_open = now - timedelta(days=days_open)
        date_low = now - timedelta(days=days_low)
        date_benign = now - timedelta(days=days_benign)
        ...
        logger.info(f"🧹 Limpieza completada: {total_deleted} alertas eliminadas")
        ...
        return {
            ...
            "message": f"Se eliminaron {total_deleted} alertas antiguas"
        }
    except Exception as e:
