...
class SnortIntegration:
    def __init__(self, api_url: str = "http://localhost:8000"):
        ...
        self.max_retries = 10
        self.retry_delay = 5
        self.initialized = False  # Flag para saltar al final en primera lectura
    ...
    def parse_snort_alert(self, alert_text: str) -> Dict:
        ...
        # Parsear timestamp: 11/09-00:03:56.046650 (hora local de Santiago)
        timestamp_match = re.search(r'^(\d{2})/(\d{2})-(\d{2}:\d{2}:\d{2}\.\d+)', alert_text)
        if timestamp_match:
            month = timestamp_match.group(1)
            day = timestamp_match.group(2)
            time_part = timestamp_match.group(3).split('.')[0]  # Sin microsegundos
            current_year = datetime.now().year
            # Crear timestamp en formato ISO 8601 sin zona horaria (hora local)
            alert_data["timestamp"] = f"{current_year}-{month}-{day} {time_part}"
        else:
    ...
    def map_severity(self, priority: str) -> str:
        ...
        En Snort: priority 0 y 1 = MÁS ALTA, 2 = MEDIA, 3+ = BAJA
        """
        priority_map = {
            "0": "ALTA",      # Prioridad 0 - Ataques críticos
            "1": "ALTA",      # Prioridad 1 - Ataques críticos
            "2": "MEDIA",     # Prioridad 2 - Reconocimiento/escaneos
            "3": "BAJA"       # Prioridad 3+ - Anomalías menores
        }
    ...
    def send_to_threatguard(self, alert_data: Dict):
        ...
        try:
            ...
            response = requests.post(f"{self.api_url}/snort/alerts", json=payload, timeout=5)
            
            if response.status_code == 200:
                # Solo log para alertas ALTA
                if severity == "ALTA":
                    logger.info(f"✅ ALTA: {alert_data.get('title')} - {alert_data.get('source_ip', 'N/A')}")
                self.api_available = True
            else:
    ...
    def monitor(self, interval: float = 0.5):
        """Monitorear continuamente el archivo de alertas (cada 0.5 segundos)"""
        ...
        while True:
            try:
                # Verificar salud de API cada 30 iteraciones (30 segundos con interval=1)
                check_counter += 1
                if check_counter >= 30:
